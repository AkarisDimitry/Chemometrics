...
class FAPV21(object):
	...
	def aling(self, D=None, S=None, Nc=None, Na=None, Ni=None, Nt=None, a=None,
					area='gaussian_coeficient', shape='gaussian', non_negativity=True, 
					linear_adjust=True, SD={'mode':'mean'},	interference_elimination=False,
		...
		mu_range = np.array(mu_range) if type(mu_range) in (list, np.ndarray) else self.mu_range
		sigma_range = np.array(sigma_range) if type(sigma_range) in (list, np.ndarray) else self.sigma_range
	...
	def aling_FAPV21(self, D=None, X=None,S=None, Nc=None, Na=None, Ni=None, Nt=None, a=None, shape='gaussian',
					area='gaussian_coeficient', non_negativity=True, linear_adjust=True, SD={'mode':'mean'},
					interference_elimination=False,
					mu_range=None, sigma_range=None, save=True, v=1, 
		...
		Z = Ze
	  	...
	  	# --- Y --- #
		Y = np.array(reduce(lambda x, y:  np.concatenate( (x ,  y), axis=1), D[:Nc,:,:] ))
		...
		print( np.array(Ye).shape )
		#Ye = D[0,:,:]
		#for n in range(1, D.shape[0]): Ye = np.concatenate( (Ye ,  D[n,:,:]), axis=1)
		#Y = Ye
		#Ze = S[0][:Na, :]
		#Z = Ze
		#Lc=np.dot(Y.T, np.linalg.pinv(Z) )
		...
		plt.show()
		ERROR
		# separate loadings per sample ||  L.shape = (Nc, Na, l2)
		L = np.array([ Lc[nc*l2:(nc+1)*l2,:].T for nc in range(Nc) ])
		...
		Lmax, Amax, Da = np.zeros((Na)), np.argmax(A, axis=0), np.zeros((N, l1, l2))

		if SD['mode'] == 'mean': 								sigma = np.mean(Gcoef[:, :, 1], axis=0)
		if SD['mode'] == 'constant' and 'value' in SD: 			sigma = [SD['value'] ]*(Na+Ni)
		elif SD['mode'] == 'constant' and not 'value' in SD: 	sigma = [l2/f/30 ]*(Na+Ni)
		...
		if shape == 'original': # retains the original shape  
			...
			for na in range(Na):
				Ma = np.tensordot(S[0][na, :], self.gaussian(Lmax[na], sigma[na], l2) , axes=0)
				for nc in range(Nc): Da[nc,:,:] += Ma*A[nc, na]

			# --- Estimacion de los canales originalemente NO alineados(self.L) ya alineados(self.La).
			La = np.zeros( (N, f, l2) )
			for na in range(Na):
				for nc in range(Nc): La[nc, na, :] = self.gaussian(Lmax[na], sigma[na], l2)*A[nc, na]
		...
		if Nt > 0:
			...
			if interference_elimination == True:
				A[:,:, Na: ] = 0
			...
			if shape == 'original': # retains the original shape  
				...
				for na in range(Na):
					for nt in range(Nt): 
						La[nt+Nc, na, :] = self.gaussian(Lmax[na], sigma[na], l2)*A[nt, na]
	...
	def plot(self, names=None):
		...
		fig_all, ax_all = [], []
		names = names if type(names) == list else ['' for n in range(self.Na)]
		...
		for n in range(self.Na):
			...
			ax.plot( np.linspace(0,6.5,391), dat[0,:], color=(color[0], 0.5, 0.3 ), lw=2.5, alpha=0.6, label=names[n] )
