...
class ICOSHIFT(object):
	def __init__(self, X=None, D=None, Xc=None, Dc=None, Xa=None, Da=None, L=None, La=None, f=None, A=None, a=1, 
	...
	def aling(self, D=None, Dc=None, Da=None,
		...
		try:
			...
			else: print('ERROR :: code 000 :: ICOSHIFT.aling() :: can NOT get D or self.D as np.array type')
			if not type(X) is np.ndarray and type(self.X) is np.ndarray:	X = self.X		
		except:
			print('ERROR :: code 000 :: ICOSHIFT.aling() :: can NOT get D or self.D')
		...
		Dc, ints, ind, target = self.aling_ICOSHIFT_2D(xt='median',  X=self.Dc)
		...
		if type(Da) == type(None): 	
			if type(self.D) == type(None): 	
				...
				Da = self.Uncompress_2WAY(Dc=Dc, shape=self.X.shape)
			elif len(self.D.shape) == 4:
				Da = self.Uncompress_3WAY(Dc=Dc, shape=self.X.shape)
			elif len(self.D.shape) > 4:
				Da = self.Uncompress_nWAY(Dc=Dc, shape=self.X.shape)

		if save: 
			self.Dc = Dc
			self.Da = Da

		return Da, target, ints
...
# TODO
# 1 incorporar 2 canales no alineados en problemas de 3 vias  (compress + uncompress)
# 2 incorporar n canales no alineados en problemas de m > n vias (compress + uncompress)
# 3 cambiar nombre compress / uncompress a fold / unfold