...
class aling_model(tk.Frame):
    def __init__(self, data, parent):
        ...
        self.button_stimate = tk.Button(self.label_tab00_model_top, text="ADD mode",
                                  command=self.loadings_estimation_plot, bg='#AAAAAA', font=("Verdana", 10))
    ...
    def loadings_estimation_plot(self, event=None, parent=None, mode=None): 

        def act_plot(self, ):
            try:
                fig.clear()
                self.data.plot_loadings(aligned=False, mode=[mode], fig=fig)
                canvas.draw()
        ...
        def loading_estimate( ):
            if combo_stimate.get() == 'PARAFAC':
                parafac = PARAFAC()
                self.data.inyect_data(parafac)
                self.data.L[mode] = parafac.loading_stimation()[mode].T
                act_plot(self,)
        ...
        def open_loading_file(event=None):
            ...
            if input_file_name:
                ...
                self.data.L[mode] = np.loadtxt( file_name )
                act_plot(self, )
        ...
        def save_loading_file(event=None):
            try:
                input_file_name = tkinter.filedialog.asksaveasfilename(defaultextension=".txt",
                                                       filetypes=[("All Files", "*.*"), ("Text Documents", "*.txt")])
                if input_file_name:
                    figure_new, data_new = self.data.plot_loadings(aligned=False, mode=[mode], fig=None)
                    file_name = input_file_name
                    self.data.save_text(file=input_file_name, data=data_new, delimiter='\t')
